...
from Post_Process_EE import plot_field_profile_real
...
def Coupled_Wave_Solver_EEDFB(alpha_m, delta_m, kappa_DFB, zeta_DFB, L, l, L_trans, rR, rL, asurf_DFB, 
    ...
    ### For export
    Guided_export = np.zeros((len(alpha_m), num_Z), dtype=object)
    ...
    alpha_surf = np.zeros(len(alpha_m), dtype=object)
    ...
    with warnings.catch_warnings():
        ...
        ### These are variant on wavelength. Need to actually fix this for SE.
...
def Coupled_Wave_Solver_SEDFB(alpha_m, delta_m, kappa_DFB, zeta_DFB, L, l, L_trans, rR, rL, asurf_DFB, 
                              Lambda, num_Z, kappa_DBR, zeta_DBR, asurf_DBR, Gamma_ele, 
                              cleave_phase_shift, pi_phase_shift, r_DFB_DBR, plot_fields, kappa_trans, zeta_trans, asurf_trans):
    ...
    with warnings.catch_warnings():
        ...
        params_.asurf_trans = asurf_trans[0]
        params_.kappa_trans = kappa_trans
        params_.zeta_trans = zeta_trans
        
        params_.kappa3 = params_.kappa_trans * Gamma_ele
        params_.zeta3 = params_.zeta_trans * Gamma_ele
        params_.asurf3 = asurf_trans * Gamma_ele
        params_.delta_trans = 0
