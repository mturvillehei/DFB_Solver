...
def find_results_Args(params):
    ...
    ### index that = 17)
    i_r = np.argmin(np.abs(params['params_indices'][0] - i_c))
    ...
    j_r = np.argmin(np.abs(params['params_indices'][1] - j_c))
        ### This won't happen if the results_value is in the sweep results
    ...
    k_r = np.argmin(np.abs(params['params_indices'][2] - k_c))
        ### This won't happen if the results_value is in the sweep results
    ...
    ### n_c is the index in the original values (e.g. cladding_thickness)
    ### n_r is the index in the results (e.g. Pmax)
    return i_c, j_c, k_c, i_r, j_r, k_r
...
def default_Calculations(Data, sweep):
    ...
    ### First triplet is the indices within the original set (i.e. cladding thickness, grating height, etc)
    ### Second triplet is the indices within the results (i.e. Pmax, delta, etc)
    ...
    alpha_i = alpha_w + alpha_bf ### Internal loss = waveguide loss + backfilling loss = empty cavity loss + (estimated) core loss + backfilling loss.
    alpha_opt = np.where(valid_mask, (2 * alpha_m) + alpha_i, np.NaN) ### Pptical loss
    ...
    return (Data['rR'], 
            ...
            alpha_opt[results_indices[3], results_indices[4], results_indices[5], mode_index],
