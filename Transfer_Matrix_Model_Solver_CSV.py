...
def Solve_EE(duty_cycle, L, l, wavelength, Lambda, derived_values, rR, rL, num_Modes, num_Z, cleave_phase_shift, pi_phase_shift, r_DFB_DBR):
    ...
    # Derived Values:
    # DFB:
    #     deltak: {format_value(deltak_DFB)}
    #     alpha: {format_value(alpha_DFB)}
    #     asurf: {format_value(asurf_DFB)}
    #     kappa: {format_value(kappa_DFB)}
    #     zeta: {format_value(zeta_DFB)}

    # DBR:
    #     deltak: {format_value(deltak_DBR)}
    #     alpha: {format_value(alpha_DBR)}
    #     asurf: {format_value(asurf_DBR)}
